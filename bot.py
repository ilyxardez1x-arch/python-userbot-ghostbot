...
# ============================================
FULL_MENU = """
👻 **GHOST BOT v18.0 — ПОЛНОЕ МЕНЮ** 👻
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👑 **Создатель:** [@mo1chu](https://t.me/mo1chu)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📌 **ЧТО ЭТО ЗА БОТ?**
  Многофункциональный userbot для Telegram с AI-автоответчиком,
  спамом, играми, анимацией текста, мониторингом каналов и кучей
  других полезных функций. Работает от твоего аккаунта!

🤖 **AI-АВТООТВЕТЧИК:**
  `!ai вкл` — включить (отвечает в личные сообщения)
  `!ai выкл` — выключить
  `!ai статус` — показать, включён ли AI сейчас
  
  🔄 **Автоматическая ротация 3 AI:**
  • Gemini (500-1500 запросов/день)
  • Groq (1000 запросов/день)
  • Cerebras (14 400 запросов/день!)
  Если один кончается — следующий подхватывает

🧠 **УПРАВЛЕНИЕ ПАМЯТЬЮ AI:**
  `!мозг` — информация о памяти
  `!мозг очистить` — стереть память для этого чата
  `!мозг показать` — показать последние 5 сообщений

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📨 **СПАМ-КОМАНДЫ:**
  `!с [кол] [мс] [текст]` — обычный спам (сообщения остаются)
  `!сг [кол] [мс] [текст]` — скрытый спам (сообщения удаляются)
  `!стоп` — остановить любой спам
  `!уд [кол]` — удалить свои последние N сообщений в чате

👿 **ТРОЛЛИНГ-СПАМ:**
  `!троль [кол] [мс]` — прогнать ВСЕ 55+ цепочек подряд
  Пример: `!троль 1 170` — 1 раз все цепочки с задержкой 170мс
  `!троль стоп` — остановить троллинг
  🔫 Триггер "банан" в любом чате запускает `!троль 1 170`!

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 **КОПИРОВАНИЕ СООБЩЕНИЙ:**
  `!к [кол-во]` — скопировать последние N сообщений
  `!к1` (ответом) — скопировать 1 сообщение, на которое ответил

🎭 **СТИКЕРЫ:**
  `!+стикер` (ответом на стикер) — добавить стикер в коллекцию
  `!стикеры` — показать все сохранённые стикеры с номерами
  `!ст [кол] [мс] [номер]` — спам стикером из коллекции

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔊 **ГОЛОСОВЫЕ СООБЩЕНИЯ (через AI):**
  `!гс [текст]` — озвучить текст голосом echo (по умолчанию)
  `!гс [голос] [текст]` — выбрать другой голос
  Голоса: `alloy`, `echo`, `fable`, `onyx`, `nova`, `shimmer`

✨ **АНИМАЦИЯ ТЕКСТА:**
  `!текст [текст]` — плавное появление (символы → текст)
  `!текст2 [текст]` — хаотичное мерцание (символы похожи на русские)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎮 **ИГРА В СУЕФА (Камень-Ножницы-Бумага):**
  `!суефа` — начать игру в ЛС или группе
  Правила: игроки по очереди пишут `камень`, `ножницы` или `бумага`
  Сообщения с выбором автоматически удаляются

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📡 **МОНИТОРИНГ КАНАЛОВ:**
  `!монитор` — статус и настройки
  `!монитор вкл` — включить мониторинг
  `!монитор выкл` — выключить
  `!монитор слова [слово1, слово2]` — установить ключевые слова
  `!монитор каналы [@канал1, @канал2]` — указать каналы
  `!монитор каналы все` — отслеживать все каналы

🔄 **АВТОСПАМ НА КАНАЛЫ (под постами):**
  `!автоспам [канал] [кол] [мс] [текст]` — создать задание
  Пример: `!автоспам @channel 5 1000 круто`
  `!автоспамы` — список всех активных заданий
  `!автостоп [ID]` — отключить автоспам по ID

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👤 **ПОЛЕЗНЫЕ КОМАНДЫ:**
  `!профиль [@юзер]` — информация о пользователе
  `!чат` — информация о текущем чате
  `!дата` — текущая дата и время
  `!айди` — ID текущего чата
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ℹ️ **ИНФОРМАЦИЯ:**
  `!обомне` — информация о создателе
  `!обскрипте` — информация о скрипте
  `!меню` — это меню
...
async def ai_handler(event):
    ...
    try:
        async with event.client.action(chat, 'typing'):
            ...
            # 1. GEMINI
            if not ai_reply and gemini_client:
                print("⏳ Пробую Gemini...")
                models_to_try = []
            ...
            # 3. CEREBRAS
            if not ai_reply and cerebras_client:
